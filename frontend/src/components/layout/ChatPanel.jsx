/**
 * SYNAPSIX ERP — ChatPanel v3
 * Chat en tiempo real vía WebSocket (Django Channels).
 * Mantiene historial local en memoria por sesión.
 * NO desplaza el layout — portal fijo esquina inferior derecha.
 */
import { useState, useRef, useEffect, useCallback } from 'react'
import { createPortal } from 'react-dom'
import {
  MessageSquare, X, ArrowLeft, Send, Search,
  Phone, Wifi, WifiOff, Loader2,
} from 'lucide-react'
...
const Avatar = ({ contact, size = 'md' }) => (
  <div className="relative flex-shrink-0">
    <div className={clsx(
      ...
      'rounded-xl bg-synapsix-surface-3 border border-synapsix-border flex items-center justify-center font-bold text-synapsix-text-2'
    )}>
...
function ChatPanelContent() {
  const {
    isOpen, close, contacts, setContactStatus,
    messages, addMessage,
  ...
  const [typing, setTyping]     = useState(null) // nombre del contacto que escribe
  ...
  const handleWsMessage = useCallback((msg) => {
    switch (msg.type) {
      case 'chat_message': {
        if (msg.mine) return // ya está en el store local
        const contactId = msg.from_user
  ...
  }, [addMessage, setContactStatus, user?.id])
  ...
  const handleSend = () => {
    ...
    // Mostrar inmediatamente (optimistic)
    addMessage(activeContactId, {
      id: Date.now(), text, time: fmtTime(now), mine: true,
    })
  ...
  const handleTyping = (val) => {
    setInput(val)
    if (activeContactId) {
      send({ type: 'typing', is_typing: val.length > 0 })
    }
  }
          ...
          <div className="border-b border-synapsix-border flex-shrink-0">
            <div className="flex items-center justify-between px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                {/* WS badge */}
                <span className={clsx('w-2 h-2 rounded-full', wsStatus === 'connected' ? 'bg-emerald-400' : wsStatus === 'connecting' ? 'bg-amber-400 animate-pulse' : 'bg-red-400')} title={wsStatus} />
              </div>
              <button onClick={close} className="text-synapsix-muted hover:text-synapsix-text-2 p-1 rounded-lg hover:bg-synapsix-surface-2 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            {/* Mi estado */}
                ...
                return (
                  <div key={status}>
                    ...
                    {group.map(contact => {
                      ...
                      return (
                        <button key={contact.id} onClick={() => openChat(contact.id)}
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between">
                              ...
                              {unread > 0 && (
                                <span className="w-5 h-5 rounded-full bg-blue-500 text-white text-[9px] font-bold flex items-center justify-center flex-shrink-0">{unread}</span>
                              )}
                            </div>
        ...
        {activeContactId && (
          <>
            ...
            <div className="border-t border-synapsix-border p-3 flex gap-2 flex-shrink-0">
              <input type="text" value={input} onChange={e => handleTyping(e.target.value)}
...
export default function ChatPanel() {
  ...
  return (
    <>
      <button id="btn-chat" onClick={toggle}
        className={clsx(
